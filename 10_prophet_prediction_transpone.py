...
file_list = []
for file in glob.glob(f'{output_files}/10_trend_forecast*.csv'):
    #print(file)
    #print('-----')
...
for trend_file in file_list:
    ...
    #print(df_toprint2)
    print('---------------------------------------------------------')
    pd.read_csv(f'{trend_file}', header=None, sep='\t', low_memory=False).T.to_csv(f'{output_files}/11_trend_forecast_{i}.csv', header=False, index=False, sep='\t')
    i += 1


files = glob.glob('output_data/11_trend_forecast_*.csv')
#print(files)
...
files_to_remove = glob.glob('output_data/11_trend_forecast_*.csv')
